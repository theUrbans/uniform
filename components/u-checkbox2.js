...
let UCheckbox = class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
    this.uChange = createEvent(this, "uChange", 1);
    this.label = '';
    this.checked = false;
    this.size = 'medium';
    this.disabled = false;
  ...
  render() {
    return (h("label", { class: {
        ...
        visible: !!this.label,
        hidden: !this.label
      } }, this.label), h("input", { type: "checkbox", ref: (cb) => (this.checkbox = cb), checked: this.checked, onInput: this.handleInput, disabled: this.disabled }), h("span", { class: {
  ...
  static get style() { return uCheckboxCss; }
};
UCheckbox = /*@__PURE__*/ proxyCustomElement(UCheckbox, [1, "u-checkbox", {
    "label": [1],
    "checked": [1028],
    "size": [1],
    "disabled": [4],
...
function defineCustomElement() {
  ...
  const components = ["u-checkbox"];
  components.forEach(tagName => { switch (tagName) {
    case "u-checkbox":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, UCheckbox);
      }
      break;
  } });
}

export { UCheckbox as U, defineCustomElement as d };
