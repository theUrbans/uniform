...
export class UFloatingmenu {
  ...
  private onPointerDown = (event: PointerEvent) => {
    ...
    this.drag = true;
    this.dragoffset = { x: event.pageX - this.node.offsetLeft, y: event.pageY - this.node.offsetTop };
  };
  private onPointerUp = () => {
    this.drag = false;
  };
  private onPointerMove = (event: PointerEvent) => {
    ...
    else if (event.pageX - this.dragoffset.x + this.node.clientWidth > window.innerWidth) newleft = `${window.innerWidth - this.node.clientWidth}px`;
    else newleft = `${event.pageX - this.dragoffset.x}px`;

    if (event.pageY - this.dragoffset.y < 0) newtop = '0px';
    else if (event.pageY - this.dragoffset.y + this.node.clientHeight > window.innerHeight) newtop = `${window.innerHeight - this.node.clientHeight}px`;
    ...
    return (
      <Host>
        <div ref={e => (this.node = e)} class="button">
          ...
          <nav style={{ display: this.isOpen ? 'block' : 'none' }} tabindex={0} onBlur={() => (this.isOpen = false)}>
            <ul>
              {this.menu.map(item => {
                const depthLevel = 0;
                return (
